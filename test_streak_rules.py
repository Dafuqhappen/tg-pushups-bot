...
# Рекорд 3, потом стрик развалился, на шестой день — подарок.
# Период обрываем на дне подарка: дальше обновлённую амнистию уже может
# потратить следующий пропуск, и мы проверяли бы не то.
st = run([4, 4, 4, 0, 0, 0])
check("подарок поднимает стрик до рекорда", st.current, 3)
check("подарок отмечен в состоянии", st.gift_today, 3)
check("подарок обновляет месячную амнистию", st.amnesty_month, None)

# Обновлённая амнистия реально работает на следующий день после подарка
st = run([4, 4, 4, 0, 0, 0, 0])
check("после подарка пропуск гасится амнистией", st.current, 3)
...
# день подарка бесплатный: ресурсы отдыха в этот день не списываются
st = run([4] * 5 + [0], best_floor=0)
check("в день подарка амнистия не тратится", st.amnesty_month, None)

# Заморозка тоже остаётся на месте: 30 дней подряд заканчиваются накануне
# подарка, а пропуск приходится ровно на его день.
st = run([4] * 30 + [0], start=_date(2026, 5, 7))
